defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    test "the next question is randomly selected", %{quiz: quiz} do
      ...
      other_template = eventually_pik_other_template(quiz, first_template)
  ...
  defp eventually_pik_other_template(quiz, template) do
  ...
  defp answer_question(quiz, answer) do
    response = Response.new(quiz, "example@example.com", answer)
